...
namespace USM
{
	...
	public class SlotGame : MonoBehaviour
	{
		...
		public bool IsApplyOnBet;   // if is true then work as orignal package if false then not count in bet.......
		public int LineMin;
		public int Line;
		...
		public bool IsSymbolSizeChange; // if is true then work as orignal package if false then not count in bet.......
		...
		{
			if (IsSymbolSizeChange)
			{
				SymbolSize = value; ReSizeSymbols();//our changes apply.......
			}
			else
			{
				SymbolSize = value; RepositionSymbols();//orignal package.......
			}
		}
		...
		public void ReSizeSymbols()
		{
			...
			for (int x = 0; x < Reels.Count; ++x)
			{
				...
				{
					//Reels[x].SymbolPos[y] = new Vector3(0, fYStart, 0);
					Reels[x].Symbols[y].GetComponent<RectTransform>().sizeDelta = new Vector2(SymbolSize, SymbolSize);//Reels[x].SymbolPos[y];
																													  //fYStart += (float)(SymbolSize + MarginSize);
				}
			}
		}
		...
		public void MarkerCreate()
		{
			...
			for (int i = 0; i < Lines.Count; ++i)
			{
				if (i < Half) sortList[i].go.transform.localPosition = new Vector3(-950.0f + ((i % 2 == 1) ? 50 : 0), -Quad * 22.0f + i * 30.0f, 0);
				else sortList[i].go.transform.localPosition = new Vector3(950.0f - ((i % 2 == 0) ? 50 : 0), -Quad * 22.0f + (i - Half) * 30.0f, 0);
			}
		}
		...
		public void BetSetPlusMinus(int _Bet)
		{
			//Debug.LogError("_Bet plus minus:" + _Bet);
			Bet = _Bet;
			if (Bet >= BetTable.Count) Bet = BetTable.Count - 1;
			if (Bet < 1) Bet = 0;
